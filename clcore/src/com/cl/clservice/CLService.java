...
public class CLService extends Service {
	...
	public static final String ACTION = "com.cl.clservice.action.clservice";
	...
	// 包名
	public static final String CL_PACKAGE = "package";
	// SDK版本号
	public static final String CL_SDK = "sdk";
	// 渠道号
	public static final String CL_CHANNEL = "channel";

	private ICL mgr = null;
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent != null && intent.getExtras() != null) {
			...
			case ACTION_VALUE_REGISTER: {
				...
				String pkg = intent.getStringExtra(CL_PACKAGE);
				byte sdk = intent.getByteExtra(CL_SDK, (byte) 1);
				if (mgr != null)
			...
			case ACTION_VALUE_ENABLE: {
				String pkg = intent.getStringExtra(CL_PACKAGE);
				byte sdk = intent.getByteExtra(CL_SDK, (byte) 1);
				String channel = intent.getStringExtra(CL_CHANNEL);
	...
	// 载入dex类
	@SuppressLint("NewApi")
	private ICL loadDex(String jarpath) {
		final File optimizedDexOutputPath = getDir("clservice",
		...
		try {
			...
			Class libClazz = null;
			libClazz = cl.loadClass("com.cl.clservice.core.CLMgr");
			lib = (ICL) libClazz.getConstructor(Context.class)
					.newInstance(this);
		} catch (Exception e) {
			Log.e(TAG, "clservice load jar error!!!");
		}
		if (lib == null) {
	...
	private String initDexjar() {
		...
		try {
			...
			for (String f : files) {
				if (f == null || !f.contains("clcoredex-")) {
					continue;
				}
